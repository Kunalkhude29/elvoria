...
export default function PopularCategories() {
    return (
        <section className="container py-16">
            <h2 className="text-3xl font-serif text-center text-charcoal mb-12">Popular Categories</h2>

            <div className="flex flex-wrap justify-center gap-8 md:gap-12">
                {CATEGORIES.map((cat) => (
                    <Link key={cat.name} href={cat.href} className="group flex flex-col items-center">
                        <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border border-gray-100 group-hover:border-gold transition-colors duration-300">
                            <Image
                                src={cat.image}
                                alt={cat.name}
                                fill
