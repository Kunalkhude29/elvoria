...
export function WishlistProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const savedWishlist = localStorage.getItem('elvoria_wishlist');
        const savedCategory = localStorage.getItem('elvoria_wishlist_category');
        const savedNavTab = localStorage.getItem('elvoria_active_nav_tab');
        if (savedCategory) {
    ...
    useEffect(() => {
        if (isLoaded) {
            localStorage.setItem('elvoria_wishlist', JSON.stringify(wishlist));
        }
    }, [wishlist, isLoaded]);

    const addToWishlist = (newItem: WishlistItem, sourcePath: string = '/women') => {
        setLastAddedCategory(sourcePath);
        localStorage.setItem('elvoria_wishlist_category', sourcePath);
        setWishlist(prev => {
    ...
    const setActiveNavTab = (tab: string) => {
        ...
        localStorage.setItem('elvoria_active_nav_tab', tab);
    };
