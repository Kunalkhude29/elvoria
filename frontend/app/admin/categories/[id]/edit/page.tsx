...
export default function EditCategoryPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const fetchCategory = async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/categories/${params.id}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let heroImageUrl = formData.image;
            ...
            const updateRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/categories/${params.id}`, {
                method: 'PUT',
    ...
    return (
        <div>
            <h1 className="text-3xl font-serif font-bold text-charcoal mb-8">Edit Category</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 flex flex-col items-center justify-center relative hover:bg-gray-100 transition duration-200">
                            ...
                            {imagePreview ? (
                                <img src={imagePreview.startsWith('blob') || imagePreview.startsWith('http') || imagePreview.startsWith('/uploads') ? imagePreview : `/${imagePreview}`} alt="Preview" className="w-32 h-32 object-cover rounded-full shadow-sm border border-gray-200" />
                            ) : (
